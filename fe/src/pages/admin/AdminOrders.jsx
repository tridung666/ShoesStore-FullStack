  ...
  return (
    <PageWrapper>
      <div className="max-w-6xl mx-auto py-10">
        <h1 className="text-4xl font-extrabold text-center text-green-700 mb-8 drop-shadow">Order Management</h1>
          ...
          {orders.map((order) => (
            <div key={order._id} className="bg-white border border-green-100 rounded-2xl shadow-lg p-8">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4">
                <div>
                  <div className="font-bold text-lg text-green-700 mb-1">Order ID: <span className="font-mono">{order._id}</span></div>
                  <div className="text-gray-700">User: <span className="font-semibold">{order.userId?.name || "N/A"}</span></div>
                  <div className="text-gray-700">Phone: <span className="font-semibold">{order.phone || "N/A"}</span></div>
                  <div className="text-gray-700">Address: <span className="font-semibold">{order.deliveryAddress}</span></div>
                </div>
                <div className="mt-4 md:mt-0 text-right">
                  <div className="text-xl font-bold text-green-700 mb-1">Total: ${order.totalPrice}</div>
                  <div className="text-gray-600">Date: {new Date(order.orderDate).toLocaleDateString()}</div>
                  <div className="mt-2">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {order.products.map((p, idx) => (
                    <div key={idx} className="border border-gray-200 rounded-lg bg-gray-50 p-4 flex flex-col md:flex-row md:items-center md:gap-6">
                      <div className="flex-1">
                        <div><span className="font-semibold">Product:</span> {p.productId?.name || p.productId}</div>
                        <div><span className="font-semibold">Size:</span> {p.size}</div>
                        <div><span className="font-semibold">Quantity:</span> {p.quantity}</div>
