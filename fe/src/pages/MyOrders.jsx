...
const MyOrders = () => {
  ...
  // Refetch orders when user changes (fix bug: must F5 to see orders after switching account)
  useEffect(() => {
    refetch();
  }, [user, refetch]);
          ...
          {orders.map((order) => (
            <div key={order._id} className="bg-white border border-green-100 rounded-2xl shadow-lg p-8">
              <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4">
                <div>
                  <div className="font-bold text-lg text-green-700 mb-1">Mã đơn: <span className="font-mono">{order._id}</span></div>
                  <div className="text-gray-700">Tên: <span className="font-semibold">{order.userId?.name || "N/A"}</span></div>
                  <div className="text-gray-700">SĐT: <span className="font-semibold">{order.phone || order.userId?.phone || "N/A"}</span></div>
                  <div className="text-gray-700">Địa chỉ: <span className="font-semibold">{order.deliveryAddress}</span></div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {order.products.map((p, idx) => (
                    <div key={idx} className="border border-gray-200 rounded-lg bg-gray-50 p-4 flex flex-col gap-1">
                      <div><span className="font-semibold">Tên sản phẩm:</span> {p.productId?.name || p.productId}</div>
                      <div><span className="font-semibold">Size:</span> {p.size}</div>
                      <div><span className="font-semibold">Số lượng:</span> {p.quantity}</div>
                      <div><span className="font-semibold">Giá:</span> ${p.price}</div>
                    </div>
                  ))}
                </div>
              </div>
